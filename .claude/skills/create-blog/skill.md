...
Use this skill when the user provides a markdown blog file path and wants to generate a complete HTML blog post following Machine Experience (MX) patterns.

**Important:** This skill generates HTML from markdown drafts. The markdown source should be in `docs/structure/` or `docs/structure/blog-drafts/`, and the generated HTML will be placed in `outputs/bible/blogs/mx/` for review.

## Arguments

- **Blog file path** (required): Path to markdown file containing blog post (typically in `docs/structure/` or `docs/structure/blog-drafts/`)

## Blog Workflow Context
...
| **in-review** | HTML generated, undergoing review    | `outputs/bible/blogs/mx/` (symlinked as `blogs/mx/`) |
| **published** | Live on allabout.network             | `outputs/bible/blogs/mx/` + web server |
| **archived**  | No longer current but preserved      | `outputs/bible/blogs/mx/` with archived state |

### Required Markdown Frontmatter

All markdown blog files MUST include YAML frontmatter with these fields:
...
publication-date: ""                  # Date published to web (empty until published)
description: "Brief summary for meta description"
...
### Step 12: Update Blog State Metadata

**CRITICAL:** After generating HTML, update metadata to reflect "in-review" state.

**Update Markdown Source File:**

1. Read the original markdown file
2. Update YAML frontmatter:
   - Change `blog-state: "draft"` to `blog-state: "in-review"`
   - Add or update `date: "YYYY-MM-DD"` to today's date
3. Save the updated markdown file

**Add HTML State Tracking Meta Tags:**

The generated HTML file MUST include these meta tags in the `<head>` section immediately after viewport and charset:
...
- `blog-state`: Always "in-review" (newly generated HTML)
- `blog-draft-date`: Date from markdown frontmatter `date` field (or creation date if not specified)
- `blog-review-date`: Today's date (YYYY-MM-DD)
- `blog-publication-date`: Empty string (will be filled when published)
...
- Review status: final-committee-review
- Location: outputs/bible/blogs/mx/ (symlinked as blogs/mx/)
- Markdown state: Updated to "in-review"
...
6. **SVG/XML validation**: Validate all extracted SVG files

   ```bash
   # Validate all SVG files for this blog post
   for svg in outputs/bible/blogs/mx/[blog-basename]-*.svg; do
       xmllint --noout "$svg" 2>&1 || echo "✗ $svg has XML errors"
   done
   ```

   **Common XML errors to fix:**
   - Unencoded `&` characters in text (must be `&amp;`)
   - Unencoded `<` or `>` in text content
