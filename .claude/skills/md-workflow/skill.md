...
### Validating SVG/XML Files

**After markdown linting, validate any SVG illustrations:**

```bash
# Validate all SVG files in illustrations directory
cd packages/bible/chapters/illustrations
for svg in *.svg; do
    xmllint --noout "$svg" 2>&1 || echo "✗ $svg has XML errors"
done
```

**Common XML errors in SVG files:**

- Unencoded `&` characters (must be `&amp;`)
- Unencoded `<` or `>` in text content
- Unclosed tags or mismatched quotes

**Fix XML entity errors:**
```bash
# Replace unencoded ampersands in text elements
sed -i '' 's/\(text.*\)&\([^a-z]\)/\1\&amp;\2/g' file.svg
```

**Validate after fixing:**
```bash
xmllint --noout file.svg && echo "✓ Valid SVG"
```
