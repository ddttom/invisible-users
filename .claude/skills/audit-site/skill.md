...
4. Validate any SVG/XML files referenced in the report:

   ```bash
   # If report contains SVG diagrams or references
   for svg in packages/bible/chapters/illustrations/*.svg; do
       xmllint --noout "$svg" 2>&1 || echo "✗ $svg has XML errors"
   done
   ```

5. Fix any XML entity errors (unencoded `&` characters must be `&amp;`)
