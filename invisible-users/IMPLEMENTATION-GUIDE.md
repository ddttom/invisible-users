...
- **AI-Native.blog** (41KB) - The comprehensive blog post explaining the entire AI-native architecture
...
1. **Generate index** (functions.php):
...
1. **Add handshake script** (functions.php):
...
1. **Add headers** (next.config.js):

```javascript
// See Step 3 above for full configuration
```

1. **Add handshake** (pages/_app.js):
...
1. **Generate index** (scripts/generate-index.js):
...
1. **Add handshake** (theme.liquid, before </body>):
...
1. **Generate index** as part of build:
...
1. **Add handshake** to footer partial/template
...
1. Check for AI detection log:

```text
[AI Handshake] Detected AI agent: (if visiting with AI)
```

1. Verify meta tags injected:
...
- **Main Blog Post**: See AI-Native.blog for the complete guide
...
- Review the blog post (AI-Native.blog)
