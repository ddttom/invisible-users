...
class BrowserPool {
  constructor(context, options = {}) {
    ...
    this.context.logger.info(`Browser pool initialized with size: ${this.poolSize}`);
  }
   ...
   */
  async initialize() {
    this.context.logger.info('Initializing browser pool...');
    ...
    this.context.logger.info(`Browser pool ready with ${this.browsers.length} browsers`);
  }
  ...
  async _createBrowser(index) {
    try {
      this.context.logger.debug(`Launching browser ${index}...`);
      ...
      this.context.logger.debug(`Browser ${index} launched successfully`);
      return browserObj;
    } catch (error) {
      this.context.logger.error(`Error launching browser ${index}:`, error);
      throw error;
    }
  }
  ...
  async acquire() {
    ...
    if (this.availableBrowsers.length > 0) {
      ...
      this.context.logger.debug(`Browser ${browserObj.id} acquired (${this.availableBrowsers.length} available)`);
      return browserObj;
    }

    // Otherwise, wait for a browser to become available
    this.context.logger.debug('No browsers available, waiting...');
  ...
  async release(browserObj) {
    ...
    if (browserObj.pagesCreated > 50) {
      this.context.logger.debug(`Browser ${browserObj.id} has created ${browserObj.pagesCreated} pages, restarting...`);
    ...
    if (this.waitQueue.length > 0) {
      ...
      browserObj.inUse = true;
      this.context.logger.debug(`Browser ${browserObj.id} assigned to waiting request`);
      resolve(browserObj);
    } else {
      // Return to available pool
      this.availableBrowsers.push(browserObj);
      this.context.logger.debug(`Browser ${browserObj.id} released (${this.availableBrowsers.length} available)`);
    }
  }
  ...
  async _restartBrowser(browserObj) {
    try {
      this.context.logger.debug(`Closing browser ${browserObj.id}...`);
      await browserObj.instance.close();

      this.context.logger.debug(`Relaunching browser ${browserObj.id}...`);
      const newBrowser = await puppeteer.launch(this.launchOptions);
      browserObj.instance = newBrowser;
      browserObj.pagesCreated = 0;

      this.context.logger.debug(`Browser ${browserObj.id} restarted successfully`);
    } catch (error) {
      this.context.logger.error(`Error restarting browser ${browserObj.id}:`, error);
      throw error;
    }
  }
  ...
  async shutdown() {
    ...
    this.context.logger.info('Shutting down browser pool...');
    ...
    const closePromises = this.browsers.map(async (browserObj) => {
      try {
        if (browserObj.instance) {
          await browserObj.instance.close();
          this.context.logger.debug(`Browser ${browserObj.id} closed`);
        }
      } catch (error) {
        this.context.logger.error(`Error closing browser ${browserObj.id}:`, error);
      }
    });
    ...
    this.context.logger.info('Browser pool shutdown complete');
  }
