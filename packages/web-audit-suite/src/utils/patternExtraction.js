 ...
 */
export async function extractPatterns(results, outputDir, context, options = {}) {
  const {
    minServedScore = 70,
    minRenderedScore = 70,
    maxExamples = 5,
  ...
  context.logger.info(`Found ${highScoringPages.length} high-scoring pages for pattern extraction`);
  ...
  await generatePatternLibraryReport(patterns, outputDir, context, highScoringPages.length);
 ...
 * Generate pattern library report
 */
async function generatePatternLibraryReport(patterns, outputDir, context, pageCount) {
...
This pattern library is generated by analyzing high-scoring pages (≥70 served/rendered score) from your website audit. Each pattern category includes:

- **Real examples** from pages that score well with AI agents
- **Priority rating** indicating impact on AI agent compatibility
- **Effort estimate** for implementation complexity
- **Actionable recommendations** for applying patterns to your site

Patterns are extracted across six categories: Structured Data, Semantic HTML, Form Patterns, Error Handling, State Management, and llms.txt implementation.
...
`;
  ...
  context.logger.info(`Generated pattern library report: ${reportPath}`);
