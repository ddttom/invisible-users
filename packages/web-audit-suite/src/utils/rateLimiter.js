/**
 * Rate Limiting for the Web Audit Suite
 *
 * Provides two rate limiting strategies:
 * 1. Simple token-bucket rate limiter (legacy)
 * 2. Adaptive rate limiter that adjusts concurrency based on server responses (new)
 */
...
export class AdaptiveRateLimiter {
  ...
  async applyBackoff() {
    ...
    if (delay > 0) {
      this.context.logger.info(`⏱️  Applying backoff delay: ${this.backoffDelay}s`);
  ...
  logStats() {
    ...
    this.context.logger.info('📊 Rate Limiter Statistics:');
    this.context.logger.info(`  Total Requests: ${stats.totalRequests}`);
    this.context.logger.info(`  Rate Limited: ${stats.rateLimitedRequests} (${stats.rateLimitPercentage}%)`);
    this.context.logger.info(`  Final Concurrency: ${stats.currentConcurrency}`);
    this.context.logger.info(`  Total Adjustments: ${stats.adjustmentCount}`);

    if (stats.adjustments.length > 0) {
      this.context.logger.debug('Adjustment History:');
  ...
  reset() {
    ...
    this.context.logger.debug('Rate limiter reset to initial state');
  }
}

/**
 * Create an adaptive rate limiter from options object
 * @param {Object} context - Audit context
 ...
 */
export function createAdaptiveRateLimiter(context, options) {
  if (!options.rateLimiting || !options.rateLimiting.enabled) {
    return null;
  }

  return new AdaptiveRateLimiter(context, options.rateLimiting);
}
