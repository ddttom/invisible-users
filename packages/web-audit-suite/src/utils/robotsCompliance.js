 ...
 */
export function isUrlAllowed(robotsTxtData, url, context, userAgent = USER_AGENT) {
  ...
  try {
    ...
    context.logger.warn(`Error checking robots.txt for ${url}: ${error.message}`);
    return {
      allowed: true,
      matchedRule: null,
...
export async function checkRobotsCompliance(
  ...
  const check = isUrlAllowed(robotsTxtData, url, context);
  ...
  context.logger.warn(`robots.txt blocks access to: ${url} (rule: ${check.matchedRule})`);

  const userChoice = await promptUserForOverride(url, check.matchedRule, isFirstCheck);

  if (userChoice.quit) {
    context.logger.info('User chose to quit analysis due to robots.txt restriction');
    return {
      allowed: false,
      updatedForceScrape: forceScrape,
      quit: true,
  ...
  if (userChoice.enableForceScrape) {
    context.logger.warn('⚠️  User enabled force-scrape mode - robots.txt restrictions will be bypassed for remainder of session');
  }
