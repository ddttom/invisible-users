...
export function loadEnvConfig() {
  ...
  // Rate Limiting
  if (process.env[ENV_VARS.RATE_LIMIT_TOKENS]) {
    const tokens = parseInt(process.env[ENV_VARS.RATE_LIMIT_TOKENS], 10);
    if (!Number.isNaN(tokens) && tokens > 0) {
      if (!config.rateLimit) config.rateLimit = {};
      config.rateLimit.tokensPerInterval = tokens;
    }
  }
  if (process.env[ENV_VARS.RATE_LIMIT_INTERVAL]) {
    const interval = process.env[ENV_VARS.RATE_LIMIT_INTERVAL].toLowerCase();
    const validIntervals = ['second', 'minute', 'hour', 'day'];
    if (validIntervals.includes(interval)) {
      if (!config.rateLimit) config.rateLimit = {};
      config.rateLimit.interval = interval;
    }
  }
...
# ${ENV_VARS.RATE_LIMIT_TOKENS}=5
# ${ENV_VARS.RATE_LIMIT_INTERVAL}=second
`;
}
