...
Use the right status code. Agents rely on these to understand what happened.
...
Each step changes the URL (`/booking?step=2`). Progress is visible. State persists in the form or session, not just JavaScript.
...
Prefer: complete content on one page with anchor navigation. When paginating: include total counts, use `rel="prev/next"` link elements, keep items per page reasonable (20-50), reflect page in URL.
...
Include: item count, individual line items with quantities and prices, running totals, clear update/remove actions, checkout readiness state.
...
Include: order reference, confirmation status, payment summary, delivery details, items ordered, next actions. This page should be reachable via a stable URL for future reference.
...
Include: price, delivery timeframe, estimated dates, any cutoff times, destination context.
...
Agents see everything in one request. Humans can scan the full content. Browser find (Ctrl+F) works across everything. Screen readers navigate by heading.
...
No deceptive "from" pricing. Complete costs visible. Breakdown available but not intrusive.
...
Clear error message. Explicit recovery paths. Technical details available but not obstructing.
...
Hidden from humans with `display: none`, but visible to agents parsing the DOM. Update status spans with JavaScript based on actual session state.
...
- Summary (blockquote) — Concise description of your site and key access information
...
> Electronics retailer serving the UK market.
> For AI assistants helping customers find and purchase products.
...
- Attribution: Required, format "Source: TechStore (techstore.com)"

## Primary Documentation

- [Product Catalogue](https://techstore.com/products/): Full product listings
- [API Reference](https://api.techstore.com/docs/): REST API documentation
- [Help Centre](https://techstore.com/help/): Customer support articles
...
2. Wait minimum 1 hour before retrying
3. Check status at status.techstore.com
4. Contact api-support@techstore.com

Alternative access if page unavailable:
...
Looking for our shop? Visit techstore.com
Need help? Email help@techstore.com
Curious about this file? Learn more at llmstxt.org

## Contact

- General questions: ai-policy@techstore.com
- Abuse reports: abuse@techstore.com
- Technical issues: api-support@techstore.com
- Privacy concerns: privacy@techstore.com

## Optional Resources

- [Blog](https://techstore.com/blog/): Product news and guides
- [Reviews](https://techstore.com/reviews/): Customer reviews
...
Sitemap lists all indexable pages but provides no context about content type or how to interact. AI agents can use it to discover pages but need llms.txt for guidance.
...
test('consent banner does not block content', async ({ page }) => {
  ...
  // Even with consent banner, product info should be accessible
...
│   └── user-guides/
└── for-ai/
    ├── SKILL.md                      # Entry point - overview and index
